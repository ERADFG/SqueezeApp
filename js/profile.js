  ...
  if (profile.banned) {
    ...
    root.innerHTML = `
      <div class="sec-bar" id="susp-hdr">
        <a class="ep-back" href="/home" aria-label="Back" onclick="if(history.length>1){history.back();return false;}">${PROFILE_ICON_BACK}</a>
        <div><b>Profile</b></div>
      </div>
      <div class="susp-notice">
        <h1>Account suspended</h1>
        <p>InteractInk suspends accounts which violate the <a href="/rules">Rules</a>.</p>
      </div>`;
    return;
  }
  ...
  if (blockedByMe) {
    ...
    root.innerHTML = `
      <div class="profile-hdr${profile.banner_url ? ' banner-loading' : ''}" id="profile-hdr" style="${profile.banner_url ? `--banner-img:url('${esc(bannerUrl(profile.banner_url))}')` : ''}">
        <a class="profile-back-btn" href="/home" aria-label="Back to home">${PROFILE_ICON_BACK}</a>
    ...
    <div class="profile-hdr${profile.banner_url ? ' banner-loading' : ''}" id="profile-hdr" style="${profile.banner_url ? `--banner-img:url('${esc(bannerUrl(profile.banner_url))}')` : ''}">
      ...
      <div class="profile-hdr-top">
        ...
        <div class="profile-hdr-actions">
          ...
          ${!isOwnProfile && !session ? `<a class="follow-btn" href="/login">${t('action.follow')}</a>` : ''}
          ${isOwnProfile ? `<a class="profile-icon-btn profile-icon-btn-square" href="/insights" title="Analytics" aria-label="Analytics">${ICON_CHART}</a>` : ''}
