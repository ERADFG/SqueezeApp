...
function ttvShortsSyncActions(root, pc) {
  ...
  const repostBtn = pc.querySelector('.act.repost');
  const repostRail = shorts.querySelector('.ttv-shorts-repost');
  if (repostRail) {
    repostRail.classList.toggle('active', !!repostBtn?.classList.contains('reposted'));
    repostRail.querySelector('.ttv-shorts-count').textContent = repostBtn?.querySelector('.act-label')?.textContent || '0';
    repostRail.style.display = repostBtn ? '' : 'none';
  }
...
document.addEventListener('click', (e) => {
  ...
  else if (shortsBtn.classList.contains('ttv-shorts-repost')) {
    // Same story as reply: the repost dropdown (.rp-menu-wrap) lives
