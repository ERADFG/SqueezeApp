...
Deno.serve(async (req) => {
  ...
  try {
    ...
    return json({ allowed: false, status: "blocked", reason: "Moderation service error", detail: String(e?.message || e) }, 500);
  }
});
...
async function handleFileUpload(req: Request): Promise<Response> {
  ...
  if (uploadErr) {
    // Surfacing the real message (not just logging it) so a
    // misconfigured bucket/policy shows up immediately in the client's
    // console/toast instead of a dead-end generic string. This is a
    // pre-launch debugging aid — tighten this back to a generic message
    // once storage is confirmed working end-to-end.
    console.error("Quarantine upload failed:", uploadErr);
    return json({ allowed: false, reason: "Could not stage file", detail: uploadErr.message }, 500);
  }
  ...
  if (signErr || !signed) {
    ...
    return json({ allowed: false, reason: "Could not prepare file for scanning", detail: signErr?.message }, 500);
  }
  ...
  const insertErr = await insertPendingMediaRow({
  ...
  if (insertErr) {
    console.error("pending_media insert failed (both column-name attempts):", insertErr);
    await admin.storage.from("post-media-quarantine").remove([quarantinePath]);
    return json({ allowed: false, reason: "Could not queue file for review", detail: insertErr.message }, 500);
  }
...
/**
 * Inserts the pending_media row. The status column has, at different
 * points, been referred to as `status` in this function and
 * `moderation_status` in the SQL that set up RLS/triggers for the rest
 * of the schema (threads/comments use `moderation_status`) — a
 * documented, previously-unresolved mismatch. Rather than guess which
 * name the live table actually has, try `status` first and, if Postgres
 * reports that column doesn't exist (error code 42703), retry once with
 * `moderation_status` instead. Whichever succeeds is logged so this can
 * be hard-coded and this fallback removed once confirmed.
 */
async function insertPendingMediaRow(base: {
  storage_path: string;
  board_id: string;
  media_type: string;
  ...
  const attempt1 = await admin.from("pending_media").insert({ ...base, status: "pending" });
  if (!attempt1.error) {
    console.log("pending_media insert succeeded using column name: status");
    return null;
  }

  const isMissingColumn = attempt1.error.code === "42703" ||
    /column .*status.* does not exist/i.test(attempt1.error.message || "");
  if (!isMissingColumn) {
    return attempt1.error;
  }

  console.warn("`status` column not found on pending_media, retrying with `moderation_status`:", attempt1.error.message);
  const attempt2 = await admin.from("pending_media").insert({ ...base, moderation_status: "pending" });
  if (!attempt2.error) {
    console.log("pending_media insert succeeded using column name: moderation_status — update insertPendingMediaRow to use this directly.");
    return null;
  }
 ...
 */
async function runTextModeration(text: string): Promise<{ allowed: boolean; status: string; reason?: string; detail?: string }> {
  ...
  return {
    allowed: false,
    status: "blocked",
    reason: "Moderation service temporarily unavailable, please try again shortly.",
    detail: String((lastErr as Error)?.message || lastErr),
  };
}
