...
// Must match the DB check constraint conversations_desc_len_chk
// (description is null or char_length(description) <= 50) — this used
// to say 80, so any description between 51-80 chars passed client-side
// validation and the character counter, then hit the DB constraint and
...
async function loadChat() {
  ...
  if (!session) {
    ...
    const gate = t('chat.loginGate')
      .replace('{signup}', `<a href="/signup">${t('nav.signUp')}</a>`);
...
async function loadThread(session, root) {
  ...
  root.innerHTML = `
    <div class="chat-thread">
      <div class="chat-hdr">
        <a class="chat-hdr-back" href="/messages" aria-label="${esc(t('chat.back'))}">${ICON_BACK}</a>
...
async function loadGroupThread(session, root) {
  ...
  document.body.classList.add('chat-thread-open');
  document.getElementById('chat-sec-bar').innerHTML = `<a class="back" href="/messages" style="margin:0 10px 0 0;">&larr;</a> ${esc(conv.name)}`;
