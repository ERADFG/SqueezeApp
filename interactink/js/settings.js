    ...
    <div class="settings-menu">
      ...
      <div class="settings-panel" id="panel-account">
        ...
        <div class="settings-row" style="margin-top:16px;">
          <span class="lbl">Signed in with</span>
          <span class="val">${esc(connectedProviderLabel(session))}</span>
        </div>
        <div class="settings-row">
          <span class="lbl">Email</span>
          <span class="val">${esc(session.user.email || '')}</span>
        </div>
        <span class="pf-note" style="display:block;margin-top:6px;">
          InteractInk accounts sign in through your ${esc(connectedProviderLabel(session))} account —
          there's no separate InteractInk password. To change the email
          on file here, update it with ${esc(connectedProviderLabel(session))} instead.
        </span>
      </div>
...
// Accounts are OAuth-only now (see js/auth.js) — every session's
// identities[] entry names the provider that created it (google,
// twitter, discord, ...). Falls back to a generic label rather than
// guessing if that's ever missing (e.g. an old session predating this
// change, or the field shape changing upstream).
function connectedProviderLabel(session) {
  const provider = session?.user?.app_metadata?.provider
    || session?.user?.identities?.[0]?.provider;
  const NAMES = { google: 'Google', twitter: 'X', discord: 'Discord', apple: 'Apple' };
  return NAMES[provider] || 'your connected account';
}
