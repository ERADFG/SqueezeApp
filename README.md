...
Three pieces fix this ('serverless" is only true on a host that
actually runs `api/*.js` as Functions — see the sitemap caveat right
below):

- **`/sitemap.xml`** — a real static XML file at the project root,
  generated by `node scripts/build-sitemap.mjs` (run it locally,
  with real internet access, then commit/redeploy the result). It
  covers every static page (with hreflang alternates for all 6
  locales), the whole Help Center, every blog post, and — if it can
  reach Supabase when you run it — every `profiles`/`posts`/
  `communities`/public `lists`/`articles` row too, capped at ~5,000
  rows per table (see the comment in that script for how to scale
  past it). Re-run it whenever you add a static page/Help
  article/blog post, and periodically (a weekly cron, or by hand
  before each deploy) to pick up new signups/posts.
  `api/sitemap.js` builds the exact same XML but *live*, from a
  request — kept in sync with the script by hand, and still useful
  if you deploy this on a host that actually executes Vercel/Cloudflare-Pages
  Functions (see `vercel.json`'s `^/sitemap\.xml$` rewrite, which only
  fires if no static `sitemap.xml` file exists — Vercel's `"handle":
  "filesystem"` step serves the static file first). This project's
  `wrangler.jsonc` deploys as a plain Cloudflare Worker with static
  assets and no Worker script/`/functions` directory, so nothing
  dynamic runs there at all — on that host (or any other pure static
  host), the committed `sitemap.xml` file is the *only* thing that
  works, which is why it's checked in rather than left to
  `api/sitemap.js` alone.
...
- **Bot-only server-rendered HTML** (`api/prerender.js`) — requests to
  ...
  same content, just rendered by different means. Same platform caveat
  as the sitemap above applies — this only fires on a host that runs
  Vercel Functions.
...
**One thing you need to do after deploying:** run
`node scripts/build-sitemap.mjs https://yourdomain.com` (with your
real domain) to regenerate `sitemap.xml` pointing at your host before
you deploy — the checked-in one points at `https://interactink.com`.
Then resubmit `https://yourdomain.com/sitemap.xml` in Google Search
Console (and Bing Webmaster Tools) once it's live, so it actually gets
crawled instead of waiting to be discovered organically. If Search
Console reports the sitemap as HTML instead of XML, that means the
request never reached a real `sitemap.xml` file — check that it was
committed and actually deployed (a static host serving your 404 page
for `/sitemap.xml` is exactly what that error looks like).
